...
>('call/postCall', async (callData, thunkAPI) => {
    try {
        return await fetchJSON(`${_apiBase}/api/callmes`, {
            method: 'POST',
...
const CallSlice = createSlice({
    ...
    extraReducers: builder =>
        builder
            .addCase(postCall.pending, state => {
                state.loadingStatus = LoadingStatus.Fetching
            })
            .addCase(postCall.fulfilled, state => {
                state.loadingStatus = LoadingStatus.Fetched
            })
            .addCase(postCall.rejected, state => {
