...
>('order/getOrder', async ({ id, tel }, thunkAPI) => {
    try {
        return await fetchJSON(`${_apiBase}/api/orders/order/${idToNumber(id)}/?tel=${encodeURIComponent(tel)}`)
    } catch (e) {
...
>('order/postOrder', async (orderData: IOrderPostReq, thunkAPI) => {
    try {
        return await fetchJSON(`${_apiBase}/api/orders`, {
            method: 'POST',
